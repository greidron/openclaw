      ...
      startAccount: async (ctx: any) => {
        ...
        const handler = createNaverWorksWebhookHandler({
          ...
          deliver: async (event) => {
            ...
            if (autoThinkingDirective) {
              log?.info?.(
                `naverworks[${account.accountId}]: autoThinking applied ${autoThinkingDirective} for userId=${event.userId}`,
              );
            } else if (account.autoThinking?.enabled) {
              log?.info?.(
                `naverworks[${account.accountId}]: autoThinking enabled but no directive resolved for userId=${event.userId}`,
              );
            }
...
${inboundBody}`
        ...
        const unregisterHandlers = webhookPaths.map((webhookPath) => {
          const unregister = registerPluginHttpRoute({
            path: webhookPath,
            auth: "plugin",
            // Allow canonical callback path and trailing-slash variants via one route.
            match: "prefix",
            pluginId: CHANNEL_ID,
            accountId: account.accountId,
