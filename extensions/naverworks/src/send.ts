...
type NaverWorksOutboundContent =
  ...
  | { type: "audio"; originalContentUrl: string };

const IMAGE_MARKDOWN_RE = /^!\[[^\]]*\]\((https?:\/\/[^\s)]+)\)$/i;
const AUDIO_MARKDOWN_RE = /^\[(?:audio|voice)\]\((https?:\/\/[^\s)]+)\)$/i;
...
function buildOutboundContent(text: string): NaverWorksOutboundContent {
  const trimmed = text.trim();
  const imageMatch = trimmed.match(IMAGE_MARKDOWN_RE);
  if (imageMatch?.[1]) {
    return {
      type: "image",
      originalContentUrl: imageMatch[1],
    };
  }

  const audioMatch = trimmed.match(AUDIO_MARKDOWN_RE);
  if (audioMatch?.[1]) {
    return {
      type: "audio",
      originalContentUrl: audioMatch[1],
    };
  }
...
async function postUserMessage(params: {
  ...
  const content = buildOutboundContent(text);
  return fetch(url, {
    method: "POST",
    headers: {
